...
export default function PatentServicesPageClient() {
  ...
  const handleAiSubmit = (e: React.FormEvent) => {
    ...
    if (aiQuestion.trim()) {
      ...
      const scoredFaqs = patentFaqs.map(faq => {
          ...
          <div className="space-y-8 flex flex-col justify-center">
            ...
            {searchResult && (
              <div 
                ...
                className="p-6 rounded-xl max-w-lg animate-fade-in-up mt-4"
                style={{
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <h4 className="text-white font-nunito text-base md:text-lg font-semibold leading-snug" style={{ color: '#FFB703' }}>
                    {searchResult.question}
                  </h4>
                  <button
                    onClick={() => setSearchResult(null)}
                    className="text-white hover:text-gray-300 transition-colors ml-3 flex-shrink-0"
                  >
                    <FontAwesomeIcon icon={faChevronDown} className="w-5 h-5 rotate-180" />
                  </button>
                </div>
                <p className="text-white font-nunito text-sm md:text-base font-medium leading-relaxed">
                  {searchResult.answer}
                </p>
              </div>
            )}
